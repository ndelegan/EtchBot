...
"""
def full_grid_etch(num_mem:int, row_mem:int, street:int, grid_len:int, x_ul:float, y_ul:int, z_ul_1:float, z_ul_4:float, x_ur:float, y_ur:float, z_ur_1:float, z_ur_4:float, x_ll:float, y_ll:float, z_ll_1:float, z_ll_4:float, z_t_1:float, z_t_4:float, z_e_1:float, z_e_4:float):
    ...
    z_heights_1=Functions.get_z_heights(num_mem, z_ll_1, z_ul_1, z_ur_1,dev_coor, dst_points)# in relation to coordintes of cap1
    z_heights_4=Functions.get_z_heights(num_mem, z_ll_4, z_ul_4, z_ur_4,dev_coor, dst_points) # in relation to coordintes of cap4
    
    z_when_touching_first_membrane_1 = z_t_1 # curently hard codded
    ...
    z_when_ready_to_etch_first_4 = z_e_4 # couple of microns above the membrane, so probes are not touching it
